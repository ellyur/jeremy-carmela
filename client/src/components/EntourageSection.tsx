            ...
            {entourageData.parents.map((parent, index) => (
              <div key={index} className="text-center" data-testid={`parents-section-${index}`}>
                ...
                {parent.names.map((name, nameIndex) => (
                  <p key={nameIndex} className="text-sm md:text-lg lg:text-xl font-author text-foreground" data-testid={`parent-name-${index}-${nameIndex}`}>
                    {name}
                  </p>
                ))}
              </div>
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-4 max-w-5xl mx-auto">
            <div className="text-center">
              {entourageData.principalSponsors.men.map((name, index) => (
                <p key={index} className="text-sm md:text-lg lg:text-xl font-author text-foreground mb-1 whitespace-nowrap text-right" data-testid={`principal-sponsor-men-${index}`}>
                  {name}
                </p>
              ))}
            </div>
            <div className="text-center">
              {entourageData.principalSponsors.women.map((name, index) => (
                <p key={index} className="text-sm md:text-lg lg:text-xl font-author text-foreground mb-1 whitespace-nowrap text-left" data-testid={`principal-sponsor-women-${index}`}>
                  {name}
                </p>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:gap-8 max-w-4xl mx-auto">
            <div className="text-center" data-testid="groomsmen-section">
              ...
              {entourageData.groomsmen.map((name, index) => (
                <p key={index} className="text-sm md:text-lg lg:text-xl font-author text-foreground mb-1 text-right" data-testid={`groomsman-${index}`}>
                  {name}
                </p>
              ))}
            </div>
            <div className="text-center" data-testid="bridesmaids-section">
              ...
              {entourageData.bridesmaids.map((name, index) => (
                <p key={index} className="text-sm md:text-lg lg:text-xl font-author text-foreground mb-1 text-left" data-testid={`bridesmaid-${index}`}>
                  {name}
                </p>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 md:gap-4 max-w-5xl mx-auto">
              <div className="text-center">
                {entourageData.bestMen.map((name, index) => (
                  <p key={index} className="text-sm md:text-lg lg:text-xl font-author text-foreground mb-1 whitespace-nowrap text-right" data-testid={`best-man-${index}`}>
                    {name}
                  </p>
                ))}
              </div>
              <div className="text-center">
                {entourageData.maidsOfHonor.map((name, index) => (
                  <p key={index} className="text-sm md:text-lg lg:text-xl font-author text-foreground mb-1 whitespace-nowrap text-left" data-testid={`maid-of-honor-${index}`}>
                    {name}
                  </p>
                ))}
              </div>
        ...
        >
          <div className="text-center" data-testid="flower-girl-section">
            ...
            {entourageData.flowerGirls.map((name, index) => (
              <p key={index} className="text-sm md:text-lg lg:text-xl font-author text-foreground mb-1" data-testid={`flower-girl-${index}`}>
                {name}
              </p>
            ))}
          </div>
